...
public class Logic
    ...
    public double calculate(int size, int count) {
            int num_of_fav_simulation=0;

            for (int i = 0; i<count  ; i++)
            {    int[] B_day = new int[365];
                 Random date=new Random();
                 date.setSeed((long)i+1);

               for (int j = 0; j <size ; j++)
               {
                  int k=date.nextInt(365);

                  B_day[k]++;

                   if(B_day[k]==2){
                       num_of_fav_simulation++;
                       break;
                   }
               }
            }

        return (num_of_fav_simulation*100.00)/count;
    }
